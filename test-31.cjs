...
(async()=>{
  ...
  await test('3.1 guided tour completes rhythm, two real cards and one plan',async()=>{
    const a=await boot();
    a.click('#startSetup');await a.tick(500);assert.equal(a.state().onboarding.tourStep,'rhythmAmount');assert(a.show('#tourLayer'));
    a.fill('#income','800');a.click('#tourNext');assert.equal(a.state().onboarding.tourStep,'rhythmPeriod');
    a.click('#tourNext');assert.equal(a.state().onboarding.tourStep,'rhythmSchedule');
    a.click('#tourNext');assert.equal(a.state().onboarding.tourStep,'rhythmSave');
    a.click('#saveProfile');await a.tick();
    assert.equal(a.state().onboarding.tourStep,'firstName');assert(a.show('#converter'));
    a.fill('#itemName','Кофе');a.click('#tourNext');assert.equal(a.state().onboarding.tourStep,'firstDetails');
    a.fill('#itemPrice','100');a.fill('#itemQty','1');a.click('#tourNext');assert.equal(a.state().onboarding.tourStep,'firstAdd');
    a.click('#addItem');await a.tick(3500);assert.equal(a.state().onboarding.tourStep,'firstResult');
    a.click('#tourNext');assert.equal(a.state().onboarding.tourStep,'saveFirst');a.click('#saveCalculation');await a.tick(500);
    assert.equal(a.state().onboarding.tourStep,'firstSaved');assert(a.$('#screenFavorites').classList.contains('active'));
    a.click('#tourNext');await a.tick(500);assert.equal(a.state().onboarding.tourStep,'second');assert(a.$('#screenCurrent').classList.contains('active'));
    await a.add('Книга',200,1);assert.equal(a.state().onboarding.tourStep,'saveSecond');a.click('#saveCalculation');await a.tick(500);
    assert.equal(a.state().calculations.length,2);assert.equal(a.state().onboarding.tourStep,'selectFirst');
    const first=a.$('[data-id="'+a.state().onboarding.firstCardId+'"]');
    a.pointer(first,'pointerdown',30);await a.tick(600);a.pointer(first,'pointerup',30);
    assert.equal(a.state().onboarding.tourStep,'selectSecond');
    a.click('[data-id="'+a.state().onboarding.secondCardId+'"]');assert.equal(a.state().onboarding.tourStep,'combine');assert(a.show('#combineSelected'));
    a.click('#combineSelected');a.fill('#dialogBody input','План знакомства');a.choose('Сохранить');await a.tick(500);
    assert.equal(a.state().projects.length,1);assert.equal(a.state().onboarding.tourStep,'planResult');assert(a.$('#projectDetailView')&&!a.$('#projectDetailView').classList.contains('hidden'));
    a.click('#tourNext');await a.tick(500);assert.equal(a.state().onboarding.tourStep,'finish');assert(a.$('#screenCurrent').classList.contains('active'));
    a.click('#tourNext');assert(a.state().onboarding.tourCompleted);assert(!a.state().onboarding.tourActive);assert(!a.show('#tourLayer'));a.close();
  });
  await test('3.1 keeps returning users out of forced tour and can restart or skip it',async()=>{
    const a=await boot(saved([card('existing')]));assert(a.state().onboarding.tourCompleted);assert(!a.show('#tourLayer'));
    a.click('#settingsButton');a.click('#restartTour');a.choose('Начать знакомство');await a.tick();
    assert(a.state().onboarding.tourActive);assert.equal(a.state().onboarding.tourStep,'rhythmAmount');
    a.click('#tourSkip');assert(a.state().onboarding.tourCompleted);assert(!a.state().onboarding.tourActive);assert.equal(a.state().calculations.length,1);a.close();
  });
  await test('3.1 resumes an interrupted guided step without losing saved data',async()=>{
    const a=await boot();a.click('#startSetup');await a.tick(500);a.fill('#income','800');a.click('#saveProfile');await a.tick();
    assert.equal(a.state().onboarding.tourStep,'firstName');const persisted=JSON.stringify(a.state());a.close();
    const b=await boot(persisted);assert(b.state().onboarding.tourActive);assert.equal(b.state().onboarding.tourStep,'firstName');
    assert(b.$('#screenCurrent').classList.contains('active'));assert(b.show('#converter'));assert(b.show('#tourLayer'));b.close();
  });
  await test('3.1 action colour, aligned toolbar and keyboard focus recovery are wired',async()=>{
    ...
    assert.match(css,/--action:\s*#3568f0/);assert.match(css,/\.toolbar\s*\{[\s\S]*justify-content:\s*space-between/);assert.match(css,/body\.intro-running\.intro-handoff #mainTitle[\s\S]*visibility:\s*hidden/);
    a.close();
  });
